...
class EnsembleDQN(object):
	...
	def learn(self, index):
		...
		q = (self.behavior_nets[index](state)*action_selection).sum(1,keepdims=True)
		loss = F.smooth_l1_loss(q,target_q)
		self.optimizer_set[index].zero_grad()
		loss.backward()
		self.optimizer_set[index].step()
...
def plot_cosine_similarity(agent, n_epi, seed, env_name, batch_size, ensemble_size):
	...
	plt.matshow(ret)
	plt.title('ensemble_dqn(ours) n_epi : {}'.format(n_epi + 1))
	plt.colorbar()
	plt.savefig("./{}/plot/ensemble_dqn_ours{}_seed{}.png".format(env_name, n_epi, seed))

if __name__ == '__main__':
	seed = 1
	env_name = 'CartPole-v1'
	#env_name = 'MountainCar-v0'
	print("run ensemble_dqn_new env_name : {} seed : {}".format(env_name, seed))
